...
import type { AuthMethods, Me } from "@openapps/sdk";
...
import {
  ...
  signNostr,
  signNostrWithBunker,
  signNostrWithSecretKey,
  signSiwe,
  waitForNostrProvider,
} from "./wallet.js";

@customElement("openapps-login")
export class OpenAppsLogin extends OpenAppsElement {
   ...
   */
  @property({ type: Number, attribute: "signer-timeout" }) signerTimeout = 2000;
   ...
   */
  @property({ type: String }) variant: "inline" | "panel" = "inline";

  /** Which Nostr fallback the user has opened, if any. */
  @state() private nostrFallback: "none" | "bunker" | "nsec" = "none";
  @state() private nostrHint: string | null = null;
  /** A bunker asking the user to approve in a browser. */
  @state() private authUrl: string | null = null;
  ...
  private async loginWithWallet(): Promise<void> {
  ...
  private async loginWithNostr(): Promise<void> {
  ...
  private async loginWithBunker(event: Event): Promise<void> {
    event.preventDefault();
    const input = this.renderRoot.querySelector<HTMLInputElement>("#bunker");
  ...
  private async loginWithNsec(event: Event): Promise<void> {
    event.preventDefault();
    const input = this.renderRoot.querySelector<HTMLInputElement>("#nsec");
  ...
  override render(): TemplateResult {
    if (this.me) return this.renderSignedIn(this.me);
   ...
   */
  private frame(inner: TemplateResult): TemplateResult {
    if (this.variant !== "panel") return inner;
    return html`
      <div class="panel">
        <div class="head">
   ...
   */
  private renderNostrFallback(): TemplateResult {
    if (this.nostrFallback === "bunker") return this.renderBunkerForm();
    if (this.nostrFallback === "nsec") return this.renderNsecForm();
    return html`
      <button
        class="link"
        ?disabled=${this.busy}
  ...
  private renderBunkerForm(): TemplateResult {
    return html`
  ...
  private renderNsecForm(): TemplateResult {
    return html`
  ...
  private renderSignedIn(me: Me): TemplateResult {
    const identity =
      me.display_name ?? me.linked_accounts[0]?.caip10 ?? me.id;
    return html`
      <div class="row">
...
export function referralFromUrl(): string | undefined {
