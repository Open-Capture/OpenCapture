...
declare global {
  interface Window {
    ...
    okxwallet?: Eip1193Provider & { nostr?: Nip07Provider };
  }
}
 ...
 */
function nostrProviders(): { where: string; provider: unknown }[] {
...
function isNip07(candidate: unknown): candidate is Nip07Provider {
  const p = candidate as Nip07Provider | undefined;
  return !!p && typeof p.getPublicKey === "function" && typeof p.signEvent === "function";
}

/** The first usable Nostr signer, or null. */
export function findNostrProvider(): Nip07Provider | null {
  for (const { provider } of nostrProviders()) {
    if (isNip07(provider)) return provider;
  }
  return null;
}
 ...
 */
export async function waitForNostrProvider(timeoutMs = 2000): Promise<Nip07Provider | null> {
 ...
 */
export function availableNamespaces(): Namespace[] {
  const available: Namespace[] = [];
  if (hasEthereum()) available.push("eip155");
  if (hasNostr()) available.push("nostr");
  return available;
}
 ...
 */
export async function signNostrWithSecretKey(
  templateJson: string,
  nsec: string,
): Promise<Proof> {
  let template: NostrTemplate;
  try {
    template = JSON.parse(templateJson) as NostrTemplate;
  } catch {
  ...
  try {
    ...
    if (decoded.type !== "nsec") {
      throw new WalletError(
        `that is an ${decoded.type} key — sign-in needs the secret key, which starts with nsec1`,
      );
    }
    secret = decoded.data;
  } catch (cause) {
...
export async function connectEthereum(): Promise<string> {
  ...
  if (!provider) throw new WalletError("no Ethereum wallet found in this browser");
 ...
 */
export async function signSiwe(message: string, address: string): Promise<Proof> {
  const provider = findEthereumProvider();
  if (!provider) throw new WalletError("no Ethereum wallet found in this browser");
  try {
    ...
    if (typeof signature !== "string") throw new WalletError("wallet returned no signature");
 ...
 */
export async function signNostr(templateJson: string): Promise<Proof> {
  ...
  if (!provider) {
    throw new WalletError(
      `no Nostr signer answered (looked at ${nostrProviderNames().join(", ")})`,
    );
  }
  let template: NostrTemplate;
  try {
 ...
 */
export async function signNostrWithBunker(
  templateJson: string,
  input: string,
  options: { onAuthUrl?: (url: string) => void; timeoutMs?: number } = {},
): Promise<Proof> {
  let template: NostrTemplate;
  try {
    template = JSON.parse(templateJson) as NostrTemplate;
  ...
  if (!pointer) {
    throw new WalletError(
      "that is not a bunker:// address or a NIP-05 name — copy the connection " +
  ...
  try {
    const event = await withTimeout(
      ...
      options.timeoutMs ?? BUNKER_TIMEOUT_MS,
      "the signer did not respond — check it is running and try again",
    );
    return { type: "nostr_event", event: JSON.stringify(event) };
  } catch (cause) {
    if (cause instanceof WalletError) throw cause;
    throw new WalletError(
      cause instanceof Error ? cause.message : "the remote signer refused",
    );
  } finally {
...
function rejectionMessage(cause: unknown, fallback: string): string {
  if (cause && typeof cause === "object") {
    ...
    if (error.code === 4001) return fallback;
