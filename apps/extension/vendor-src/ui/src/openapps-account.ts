...
import type { AuthMethods, LinkResult, Me, Namespace, OtherAccount } from "@openapps/sdk";
...
/**
 * A merge awaiting confirmation.
 *
 * The proof is not kept: challenges are single-use, so the refused attempt
 * already spent it and confirming has to sign a fresh one. All that needs
 * carrying is which method to re-sign with, and what to tell the user.
 */
...
export class OpenAppsAccount extends OpenAppsElement {
  @state() private me: Me | null = null;
  @state() private enabled: AuthMethods | null = null;
  @state() private pending: PendingMerge | null = null;
  ...
  private linked(namespace: string): boolean {
   ...
   */
  private get connectable(): Namespace[] {
    return (["eip155", "nostr"] as Namespace[]).filter(
      (ns) => this.enabled?.[ns] && !this.linked(ns),
    );
  }

  /** Google links by redirect rather than by signing in the page. */
  private get canConnectGoogle(): boolean {
    return (this.enabled?.google ?? false) && !this.linked("google");
  }

  private async connectGoogle(merge = false): Promise<void> {
  ...
  private handleLinkRedirect(): void {
    let outcome;
    try {
  ...
  private async connect(namespace: Namespace): Promise<void> {
    this.blocked = null;
    await this.run(async () => {
      ...
      const proof =
        namespace === "eip155"
          ? await signSiwe(challenge.message, address as string)
      ...
      try {
        ...
        if (
          error instanceof OpenAppsError &&
        ...
        if (
          ...
          this.pending = {
            namespace,
            other: error.detail.other_account as OtherAccount,
          };
          return;
        }
        throw error;
  ...
  private async confirmMerge(): Promise<void> {
    ...
    if (!pending) return;
    if (pending.namespace === ("google" as Namespace)) {
    ...
    await this.run(async () => {
      const address =
        pending.namespace === "eip155" ? await connectEthereum() : undefined;
      const challenge = await this.sdk.auth.linkChallenge(pending.namespace, address);
      const proof =
        pending.namespace === "eip155"
          ? await signSiwe(challenge.message, address as string)
          : await signNostr(challenge.message);
      const result = await this.sdk.auth.linkVerify(challenge.challenge_id, proof, {
        merge: true,
  ...
  private afterLink(result: LinkResult): void {
  ...
  private async unlink(caip10: string): Promise<void> {
    await this.run(async () => {
  ...
  override render(): TemplateResult {
    ...
    if (!this.me) return html`<p class="muted">Loading…</p>`;
    if (this.pending) return this.renderMergePrompt(this.pending);
  ...
  private renderMergePrompt(pending: PendingMerge): TemplateResult {
    return html`
      <div class="card">
...
function middle(value: string, head = 18, tail = 8): string {
