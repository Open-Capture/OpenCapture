...
import type { Referees, ReferralCode, ReferralEarnings } from "@openapps/sdk";

import { OpenAppsElement } from "./base.js";

type Tab = "link" | "referees" | "earnings";
...
export class OpenAppsReferral extends OpenAppsElement {
  /**
   ...
   */
  @property({ type: String, attribute: "invite-url" }) inviteUrl?: string;

  @state() private info: ReferralCode | null = null;
  @state() private earnings: ReferralEarnings | null = null;
  @state() private referees: Referees | null = null;
  @state() private tab: Tab = "link";
  ...
  private renderLink(): TemplateResult {
  ...
  private renderReferees(people: Referees["referees"]): TemplateResult {
    if (people.length === 0) {
  ...
  private renderEarnings(rows: ReferralEarnings["entries"]): TemplateResult {
    if (rows.length === 0) {
      return html`<p class="muted">
