...
/** One row of the "where it went" summary. */
...
export class OpenAppsHistory extends OpenAppsElement {
  ...
  @property({ type: Boolean, attribute: "no-summary" }) noSummary = false;

  @state() private entries: LedgerEntry[] = [];
  ...
  private get visible(): LedgerEntry[] {
    if (!this.appId) return this.entries;
   ...
   */
  private get spending(): SpendGroup[] {
  ...
  override render(): TemplateResult {
    ...
    if (!this.loaded) return html`<p class="muted">Loading…</p>`;
 ...
 */
export function spendLabel(entry: LedgerEntry): string {
  const app = entry.app_name ?? entry.app_id ?? null;
  const feature = entry.ref_id ?? null;
  if (app && feature) return `${app} · ${feature}`;
  if (app) return app;
