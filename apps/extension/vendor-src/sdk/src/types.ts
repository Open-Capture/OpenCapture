...
export type LinkRedirect =
  | { status: "linked"; namespace: string; merged: boolean; credits: number }
  /** The identity is on another account; retry the link with `merge: true`. */
  | { status: "conflict"; namespace: string; balance: number }
  /** Both accounts use the same sign-in methods; combining is impossible. */
  | { status: "blocked"; namespaces: string[]; message: string }
...
export interface Packages {
  ...
  rails: { stripe: boolean; ethereum: boolean; lightning: boolean };
}
