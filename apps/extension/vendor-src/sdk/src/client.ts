...
import { defaultStore, type Session, type TokenStore } from "./storage.js";
import type {
  AuthMethods,
  Challenge,
  CreditPackage,
...
type AuthMode = "none" | "bearer" | "app+bearer";
...
export class OpenApps {
   ...
   */
  #refreshing: Promise<Session | null> | null = null;
  /** In-flight redirect-code exchange, shared by concurrent callers. */
  #completing: Promise<LoginResult | null> | null = null;
  ...
  async #request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (auth === "app+bearer" && !this.#appKey) {
      throw new OpenAppsError(
        "unauthorized",
        "this call needs an app key; construct OpenApps with { appKey }",
      );
    }
    return this.#send<T>(path, options, auth, true);
  }
  ...
  async #send<T>(
    ...
    if (response.status === 401 && auth !== "none" && mayRefresh) {
      ...
      if (refreshed) return this.#send<T>(path, options, auth, false);
    }
    ...
    if (!response.ok) {
      ...
      if (error.code === "unauthorized" && auth !== "none") this.#setSession(null);
      throw error;
    }
    return payload as T;
  }
  ...
  auth = {
     ...
     */
    methods: async (signal?: AbortSignal): Promise<AuthMethods> => {
      const result = await this.#request<{ methods: AuthMethods }>(
        "/v1/auth/methods",
        { signal },
      );
    ...
    challenge: (namespace: Namespace, address?: string, signal?: AbortSignal) =>
      this.#request<Challenge>("/v1/auth/challenge", {
        method: "POST",
    ...
    verify: async (
      challengeId: string,
      proof: Proof,
      options: { referralCode?: string; signal?: AbortSignal } = {},
    ): Promise<LoginResult> => {
      const result = await this.#request<LoginResult>("/v1/auth/verify", {
        method: "POST",
        body: {
     ...
     */
    completeRedirect: (
      options: { url?: string; hash?: string; signal?: AbortSignal } = {},
      ...
      this.#completing = (async () => {
        try {
          const result = await this.#request<LoginResult>("/v1/auth/oidc/exchange", {
            method: "POST",
            body: { code },
            signal: options.signal,
          });
    ...
    me: (signal?: AbortSignal) =>
      this.#request<Me>("/v1/me", { auth: "bearer", signal }),
    ...
    linkChallenge: (namespace: Namespace, address?: string, signal?: AbortSignal) =>
      this.#request<Challenge>("/v1/auth/link/challenge", {
        method: "POST",
        auth: "bearer",
        body: { namespace, address },
        signal,
    ...
    linkVerify: (
      challengeId: string,
      proof: Proof,
      options: { merge?: boolean; signal?: AbortSignal } = {},
    ) =>
      this.#request<LinkResult>("/v1/auth/link/verify", {
        method: "POST",
        auth: "bearer",
        body: {
          challenge_id: challengeId,
     ...
     */
    googleLinkStart: async (
      returnTo: string,
      options: { merge?: boolean; signal?: AbortSignal } = {},
    ): Promise<string> => {
      const result = await this.#request<{ auth_url: string }>(
        "/v1/auth/link/oidc/google/start",
        {
          method: "POST",
          auth: "bearer",
    ...
    completeLinkRedirect: (
      options: { url?: string; hash?: string } = {},
    ): LinkRedirect | null => {
      ...
      if (!linked && !conflict && !blockedBy && !error) return null;
      ...
      if (blockedBy) {
        ...
        return {
          ...
          namespaces,
          message:
            `That Google account belongs to another account which also has a ${names} ` +
      ...
      return {
        status: "linked",
        namespace: linked as string,
        merged: params.get("merged") === "1",
        credits: Number(params.get("credits") ?? 0),
      };
    },

    unlink: (caip10: string, signal?: AbortSignal) =>
      this.#request<{ ok: boolean }>(`/v1/auth/link/${encodeURIComponent(caip10)}`, {
        method: "DELETE",
        auth: "bearer",
        signal,
      }),
  ...
  credits = {
    balance: async (signal?: AbortSignal): Promise<number> => {
      const result = await this.#request<{ balance: number }>("/v1/credits/balance", {
        auth: "bearer",
        signal,
      });
    ...
    deduct: (
      ...
      this.#request<DeductResult>("/v1/credits/deduct", {
        method: "POST",
        auth: "app+bearer",
    ...
    history: (options: { cursor?: number; limit?: number; signal?: AbortSignal } = {}) =>
      this.#request<History>("/v1/credits/history", {
        auth: "bearer",
  ...
  payments = {
    packages: (signal?: AbortSignal) =>
      this.#request<Packages>("/v1/payments/packages", { signal }),
    ...
    stripeCheckout: (
      ...
      this.#request<StripeCheckout>("/v1/payments/stripe/checkout", {
        method: "POST",
        auth: "bearer",
        body: {
    ...
    ethDepositAddress: (packageId: string, signal?: AbortSignal) =>
      this.#request<EthDeposit>("/v1/payments/eth/deposit-address", {
        method: "POST",
        auth: "bearer",
    ...
    lightningInvoice: (packageId: string, signal?: AbortSignal) =>
      this.#request<LightningInvoice>("/v1/payments/lightning/invoice", {
        method: "POST",
        auth: "bearer",
        body: { package_id: packageId },
        signal,
    ...
    list: (signal?: AbortSignal) =>
      this.#request<{ topups: Topup[] }>("/v1/payments/topups", {
        auth: "bearer",
        signal,
      }),

    get: (topupId: string, signal?: AbortSignal) =>
      this.#request<Topup>(`/v1/payments/topups/${encodeURIComponent(topupId)}`, {
        auth: "bearer",
        signal,
      }),
    ...
    waitFor: async (
      ...
      for (;;) {
        ...
        try {
          ...
          if (TERMINAL_STATUSES.has(topup.status)) return topup;
        } catch (error) {
          // Auth and 404 are terminal; anything else may be transient.
          if (error instanceof OpenAppsError && error.code !== "network") throw error;
          if (!(error instanceof OpenAppsError)) throw error;
        }
        if (Date.now() + interval > deadline) {
          throw new OpenAppsError(
            "timeout",
            `top-up ${topupId} was still pending after the timeout`,
          );
        }
  ...
  referral = {
    code: (signal?: AbortSignal) =>
      this.#request<ReferralCode>("/v1/referral/code", { auth: "bearer", signal }),

    apply: (code: string, signal?: AbortSignal) =>
      this.#request<{ ok: boolean }>("/v1/referral/apply", {
        method: "POST",
        auth: "bearer",
        body: { code },
        signal,
    ...
    earnings: (signal?: AbortSignal) =>
      this.#request<ReferralEarnings>("/v1/referral/earnings", {
        auth: "bearer",
        signal,
      }),
     ...
     */
    referees: (signal?: AbortSignal) =>
      this.#request<Referees>("/v1/referral/referees", {
        auth: "bearer",
        signal,
      }),
  };
 ...
 */
function redirectParams(options: { url?: string; hash?: string }): RedirectParams {
...
/** The only thing callers need from a parsed redirect. */
