/**
 * The server speaks one error shape:
 * `{"error": {"code": "insufficient_balance", "message": "…"}}`.
 * Every failure this SDK raises is an `OpenAppsError` carrying that code, so
 * callers branch on a stable string rather than parsing messages or
 * memorising status codes.
 */
export type ErrorCode =
  | "bad_request"
  | "unauthorized"
  | "insufficient_balance"
  | "not_found"
  | "conflict"
  | "rate_limited"
  | "internal"
  /** The identity you tried to link is already on a different account. */
  | "identity_belongs_to_another_account"
  /** The request never reached the server (offline, DNS, CORS, abort). */
  | "network"
  /** A poll helper gave up before the top-up reached a terminal state. */
  | "timeout";

export class OpenAppsError extends Error {
  readonly code: ErrorCode;
  /** HTTP status, or 0 when the request never got a response. */
  readonly status: number;
  /** Present on `insufficient_balance`: what the user actually has. */
  readonly balance?: number;
  /**
   * The whole error object the server sent. Some conflicts carry structured
   * detail a UI has to act on — `identity_belongs_to_another_account`
   * includes the other account's id and balance so you can ask "combine
   * them?" without a second request.
   */
  readonly detail?: Record<string, unknown>;

  constructor(
    code: ErrorCode,
    message: string,
    status = 0,
    balance?: number,
    detail?: Record<string, unknown>,
  ) {
    super(message);
    this.name = "OpenAppsError";
    this.code = code;
    this.status = status;
    this.balance = balance;
    this.detail = detail;
  }

  /** True when re-authenticating could plausibly fix this. */
  get isAuthError(): boolean {
    return this.code === "unauthorized";
  }
}

const STATUS_CODES: Record<number, ErrorCode> = {
  400: "bad_request",
  401: "unauthorized",
  402: "insufficient_balance",
  404: "not_found",
  409: "conflict",
  429: "rate_limited",
};

/** Build an error from a response whose body may or may not be our shape. */
export function errorFromResponse(status: number, body: unknown): OpenAppsError {
  const envelope =
    body && typeof body === "object" ? (body as Record<string, unknown>).error : undefined;
  const detail =
    envelope && typeof envelope === "object" ? (envelope as Record<string, unknown>) : undefined;

  const code = (detail?.code as ErrorCode) ?? STATUS_CODES[status] ?? "internal";
  const message =
    (detail?.message as string) ?? `request failed with status ${status}`;

  // `balance is 42` — the server puts the real balance in the message so the
  // UI can say how many credits are short without a second round-trip.
  let balance: number | undefined;
  if (code === "insufficient_balance") {
    const match = /-?\d+/.exec(message);
    if (match) balance = Number(match[0]);
  }
  return new OpenAppsError(code, message, status, balance, detail);
}
